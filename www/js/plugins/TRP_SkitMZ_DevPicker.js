...
TRP_SkitDevPicker.FILE_PATH =
  TRP_SkitDevPicker.DIR_PATH + "/TRPSkitDevPickerData.json";
...
(function () {
  ...
  PluginManager.registerCommand(pluginName, "picker", function (args) {
    ...
    TRP_SkitDevPicker.startPickExp(
      args.charaName,
      args.poseName,
      args.addFlag === "true" || args.addFlag === true
    );
...
===================================*/
  ...
  /* exp picker
===================================*/
  TRP_SkitDevPicker.startPickExp = function (
    actorName,
    pose = "normal",
    addFlag = false
  ) {
  ...
  TRP_SkitExpPicker.prototype.initialize = function (
    name,
    pose = "normal",
    addFlag = false
  ) {
  ...
  TRP_SkitExpPicker.prototype.processDevSave = function () {
    ...
    TRP_SkitDevPicker.savePoseDevData(
      this._name,
      this._pose,
      this._poseDevData
    );
  ...
  TRP_SkitExpPicker.DEV_SAVE_INFO = ["ピッカー用の設定を保存しました。"];
  TRP_SkitExpPicker.SAVE_INFO = ["編集データを保存しました。"];
  ...
  TRP_SkitExpPicker.prototype.refreshMatrixSprite = function (
    cols,
    rows,
    w,
    h
...
===================================*/
  TRP_SkitExpPicker.prototype.setupData = function (noLoad) {
    ...
    while (!maxParts.equals(idxes)) {
      if (!this._addMode) {
        ...
        if (
          !this._savedExpressions.some(function (exp) {
  ...
  TRP_SkitExpPicker.prototype.clampSize = function () {
    ...
    this._thumbY = this._thumbY.clamp(
      0,
      this._baseBitmap.height - this._thumbH
    );
  };
  ...
  TRP_SkitExpPicker.prototype.setupSprite = function (sprite, data) {
    ...
    for (var i = 0; i < length; i = (i + 1) | 0) {
      ...
      partsSprite.bitmap = ImageManager.loadBust(
        this._name,
        this._pose + "_" + (i + 1) + "_" + partsId
      );
...
===================================*/
  ...
  TRP_SkitExpPicker.prototype.didSelect = function (index, realIndex) {
    if (this._selectedIndexes.contains(realIndex)) {
      ...
      this._selectedIndexes.sort((a, b) => {
        return a - b;
      });
    }
  ...
  TRP_SkitExpPicker.prototype.refreshSelectedNumNavi = function (
    refreshPage = false
  ) {
...
===================================*/
  ...
  TRP_SkitExpPicker.prototype.movePosition = function (
    dx,
    dy,
    noRefresh = false
  ) {
...
===================================*/
  ...
  TRP_SkitExpPicker.prototype._drawThumbnail = function (
    data,
    kinds,
    kind,
    bitmap,
...
===================================*/
  ...
  SubPicker.KIND_GUIDE = ["", "対象のパーツ種類を選択"];
