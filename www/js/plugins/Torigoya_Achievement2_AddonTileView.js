...
(function () {
  ...
  checkPlugin(
    ...
    "「実績アドオン:タイル表示」より上に「実績プラグイン」が導入されていません。"
  );
  ...
  (() => {
    ...
    const useCancelMessage =
      !!Torigoya.Achievement2.parameter.achievementMenuCancelMessage;

    const upstream_Window_AchievementList_initialize =
    ...
    Window_AchievementList.prototype.resizeFittingItems = function () {
      ...
      this.width = Math.min(
        this.maxCols() * this.itemWidth() +
          (this.maxCols() - 1) * this.spacing() +
          this.padding * 2,
        width
      );

      this.height = Math.max(
        Math.min(h * this.itemHeight() + this.padding * 2, height),
    ...
    const upstream_Window_AchievementList_drawAllItems =
      Window_AchievementList.prototype.drawAllItems;
    ...
    const upstream_Window_AchievementList_itemRect =
      Window_AchievementList.prototype.itemRect;
    Window_AchievementList.prototype.itemRect = function (index) {
      const item = this._data[index];
      if (item || !this.isRequireCancel())
        return upstream_Window_AchievementList_itemRect.apply(this, arguments);
      ...
      rect.y =
        Math.ceil((this.maxItems() - 1) / this.maxCols()) * rect.height -
        this._scrollY;
      return rect;
    };

    const upstream_Window_AchievementList_cursorUp =
      Window_AchievementList.prototype.cursorUp;
    ...
    const upstream_Window_AchievementList_cursorDown =
      Window_AchievementList.prototype.cursorDown;
