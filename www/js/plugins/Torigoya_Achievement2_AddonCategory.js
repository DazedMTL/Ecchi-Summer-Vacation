...
(function () {
  ...
  class Window_AchievementCategory extends Window_Command {
    ...
    makeCommandList() {
      Torigoya.Achievement2.Addons.Category.parameter.categories.forEach(
        (category) => {
          this.addCommand(
            category.name,
            "category_".concat(category.name),
            true,
            category
          );
        }
  ...
  Torigoya.Achievement2.Addons.Category.Window_AchievementCategory =
  ...
  function readCategoryName(achievement) {
    ...
    const autoCategory =
      Torigoya.Achievement2.Addons.Category.parameter.categories.find(
        (category) => {
  ...
  (() => {
    ...
    const upstream_Window_AchievementList_makeItemList =
      Window_AchievementList.prototype.makeItemList;
    Window_AchievementList.prototype.makeItemList = function () {
      upstream_Window_AchievementList_makeItemList.apply(this);
      this._data = this._data.filter((item) => {
        return (
          this._category &&
          this._category.name === readCategoryName(item.achievement)
        );
      });
    };
  ...
  (() => {
    ...
    const upstream_Scene_Achievement_create =
      Scene_Achievement.prototype.create;
    Scene_Achievement.prototype.create = function () {
      upstream_Scene_Achievement_create.apply(this);

      const rect = this.categoryWindowRect();
      this._categoryWindow = new Window_AchievementCategory(
        rect.x,
        rect.y,
        rect.width,
        rect.height
    ...
    const upstream_Scene_Achievement_listWindowRect =
      Scene_Achievement.prototype.listWindowRect;
