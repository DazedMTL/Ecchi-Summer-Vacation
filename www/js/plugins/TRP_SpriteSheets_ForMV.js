 ...
 * @help
 * TRP_SkitMZ_ExMVPatchにて階層レイヤー方式の画像形式で
...
(() => {
  ...
  TRP_SpriteSheet.load = function (file) {
    ...
    xhr.onload = this._onLoad.bind(this, file, xhr);

    xhr.onerror = this._onError.bind(this, url);
    xhr.send();
  };

  TRP_SpriteSheet._onLoad = function (file, xhr) {
    if (xhr.status < 400) {
  ...
  // Sprite
  //=============================================================================
  Sprite.prototype.setupWithSheet = function (sheet, image) {
    ...
    if (!data) {
      throw new Error(
        "spritesheet image data not found:" + sheet + ">" + image
      );
    }
